...
import { ArrowRight, Eye, EyeOff, Loader2, Lock, Phone } from "lucide-react";
...
export default function LoginModal() {
  ...
  return (
    <>
        ...
        >
          ...
          {banAppealToken ? (
            <div className="mb-5 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
              Your account is banned. Use the appeal button below to submit a review request.
            </div>
          ) : null}

          <form className="space-y-5" onSubmit={handleSubmit}>
            {errorMessage && (
              <div className="rounded-lg border border-red-500/30 bg-red-500/15 px-4 py-3 text-sm text-red-300 font-medium whitespace-pre-wrap">
                {errorMessage}
              </div>
            )}
            ...
            {!mfaToken && (
              <div className="space-y-2">
                <label className="text-sm font-semibold text-white" htmlFor="login-phone">
                  Phone number
                </label>
            ...
            {!mfaToken && (
              <div className="space-y-2">
                <label className="text-sm font-semibold text-white" htmlFor="login-password">
                  Password
                </label>
                <div className="relative">
            ...
            {mfaToken && (
              <div className="space-y-2">
                <label className="text-sm font-semibold text-white" htmlFor="login-mfa-code">
                ...
                {mfaMode === "totp_setup" && (
                  <div className="space-y-3 rounded-xl border border-[#3d6ba3]/40 bg-[#1a3a6b]/35 p-4">
                    <p className="text-xs text-[#a8c4e0]">
                      First-time setup: scan the QR code in Microsoft Authenticator.
                    </p>
                    {mfaQrCodeDataUrl && (
                      <img
                    ...
                    {mfaManualEntryKey && (
                      <p className="text-xs text-[#a8c4e0] break-all">
                        Manual key: <span className="text-white">{mfaManualEntryKey}</span>
                      </p>
                    )}
                  </div>
                )}
            ...
            <button
              type="submit"
              disabled={(!mfaToken && !formValid) || (Boolean(mfaToken) && mfaCode.trim().length !== 6) || isSubmitting}
          ...
          {banAppealToken ? (
            <div className="mt-5 flex gap-3 border-t border-[#3d6ba3]/30 pt-5">
              <button
                type="button"
                onClick={() => {
                  setBanAppealToken(null);
              ...
              <button
                ...
                className="flex-1 rounded-xl bg-white/10 px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/15"
              >
                Create account
              </button>
            </div>
      ...
      <BanAppealModal
        open={Boolean(banAppealToken)}
        appealToken={banAppealToken}
        banReason={banReason}
        onClose={() => setBanAppealToken(null)}
      />
    </>
  );
